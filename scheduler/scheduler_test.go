...
func (s *SchedulerTestSuite) TestRepeatedPortPrioritizationIsIdempotent(c *check.C) {
	scheduler := NewScheduler(-1)
	err := scheduler.CreateResourcePool("host-a", &PortResourcePool{
		Resource: "portReservation",
		PortBindingMapTCP: map[string]map[int64]string{
			defaultIP: {},
		},
	...
	requests := []ResourceRequest{PortBindingResourceRequest{
		InstanceID:   "162",
		ResourceUUID: "instance-uuid-162",
		Resource:     "portReservation",
	...
	candidates, err := scheduler.PrioritizeCandidates(requests, Context{})
	c.Assert(err, check.IsNil)
	c.Assert(candidates, check.DeepEquals, []string{"host-a"})

	_, err = scheduler.ReserveResources("host-a", false, requests)
	c.Assert(err, check.IsNil)
	...
	conflicting := []ResourceRequest{PortBindingResourceRequest{
		InstanceID:   "163",
		ResourceUUID: "instance-uuid-163",
		Resource:     "portReservation",
		PortRequests: []PortSpec{{
			PublicPort:  43968,
			PrivatePort: 80,
	...
	candidates, err = scheduler.PrioritizeCandidates(conflicting, Context{})
	c.Assert(err, check.IsNil)
	c.Assert(candidates, check.DeepEquals, []string{})
}
