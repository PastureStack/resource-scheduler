	...
	for _, host := range hosts {
		...
		for _, request := range resourceRequests {
			if rr, ok := request.(PortBindingResourceRequest); ok {
				if !portPool.ArePortsAvailableFor(rr.ResourceUUID, rr.PortRequests) {
					qualified = false
					break
				}
			}
...
func (p *PortResourcePool) ArePortsAvailable(ports []PortSpec) bool {
	return p.ArePortsAvailableFor("", ports)
}

// ArePortsAvailableFor reports whether a request can use a host's ports. A
// reservation already owned by the same resource UUID is available so that a
// repeated allocation pass remains idempotent.
func (p *PortResourcePool) ArePortsAvailableFor(uuid string, ports []PortSpec) bool {
L:
			...
			for _, port := range ports {
				if port.Protocol == "tcp" {
					for _, m := range p.GhostMapTCP {
						if m[port.PublicPort] != "" && m[port.PublicPort] != uuid {
							return false
						}
					}
				} else {
			...
			if port.Protocol == "tcp" {
				if port.IPAddress != "" {
					if ip != port.IPAddress ||
						(portMapTCP[port.PublicPort] != "" && portMapTCP[port.PublicPort] != uuid) {
						continue L
					}
					continue
				}
				if portMapTCP[port.PublicPort] != "" && portMapTCP[port.PublicPort] != uuid {
					continue L
				}
			} else {
				if port.IPAddress != "" {
					if ip != port.IPAddress ||
						(portMapUDP[port.PublicPort] != "" && portMapUDP[port.PublicPort] != uuid) {
						continue L
					}
					continue
				}
				if portMapUDP[port.PublicPort] != "" && portMapUDP[port.PublicPort] != uuid {
					continue L
				}
			}
