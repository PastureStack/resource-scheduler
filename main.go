...
func run(c *cli.Context) error {
	...
	go superviseComponent("control-plane event subscriber", func() error {
		return events.ConnectToEventStream(controlPlaneURL, accessKey, secretKey, resourceScheduler)
	}, retryForever)

	go superviseComponent("metadata watcher", func() error {
		return resourcewatchers.WatchMetadata(metadataClient, resourceScheduler, apiClient)
	}, retryForever)
...
const componentRetryDelay = 2 * time.Second

func superviseComponent(name string, runComponent func() error, retry func(time.Duration) bool) {
	for {
		err := runComponentSafely(runComponent)
		if err == nil {
			log.Warnf("%s disconnected; reconnecting", name)
		} else {
			log.Warnf("%s failed: %v; reconnecting", name, err)
		}

		if !retry(componentRetryDelay) {
			return
		}
	}
}

func runComponentSafely(runComponent func() error) (err error) {
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("component panicked: %v", recovered)
		}
	}()
...
func retryForever(delay time.Duration) bool {
	time.Sleep(delay)
	return true
}
...
func startHealthCheck(listen int, metadataClient metadata.Client, controlPlaneURL string) error {
	handler, err := newHealthHandler(metadataClient, controlPlaneURL, &http.Client{Timeout: 10 * time.Second})
	if err != nil {
		return err
	}
	...
	log.Infof("Listening for health checks on %s/healthcheck and readiness checks on %s/readiness", server.Addr, server.Addr)
	return server.ListenAndServe()
}

func newHealthHandler(metadataClient metadata.Client, controlPlaneURL string, pingClient *http.Client) (http.Handler, error) {
	...
	mux.HandleFunc("/readiness", func(response http.ResponseWriter, request *http.Request) {
		if request.Method != http.MethodGet && request.Method != http.MethodHead {
			response.Header().Set("Allow", "GET, HEAD")
			http.Error(response, "method not allowed", http.StatusMethodNotAllowed)
			return
